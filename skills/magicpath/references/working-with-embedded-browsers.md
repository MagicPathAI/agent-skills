...
This guidance applies to **projects/files**, not individual design previews. Keep the browser focused on the project canvas. Only open an individual design in the embedded browser when the user explicitly asks to see that specific design.
...
- Creating, submitting, or editing a design does not require leaving the project canvas. The new or edited design appears in that project.
- Do not open each design in its own preview page after `code submit` — verify progress with the returned `previewImageUrl` instead.
...
## Resolve a URL without opening an external browser

When an agent runs `open`, it never launches the OS browser — it returns the URL:

```bash
npx -y magicpath-ai@experimental open <projectId>
```

```json
{ "ok": true, "data": { "type": "project", "url": "https://www.magicpath.ai/files/<projectId>", "projectId": "<projectId>", "opened": false } }
```

Navigate the host's embedded browser to the returned `url`. Project and design results from other commands also carry a `url` field — prefer URLs the CLI returns; do not guess URL shapes or open a blank home page when the project is already known.

When no embedded browser is available, it cannot be controlled reliably, or the user explicitly wants their normal browser, pass `--browser`:

```bash
npx -y magicpath-ai@experimental open <projectId> --browser
```

## Browser sign-in is separate from CLI sign-in

The CLI can be authenticated while the embedded browser is not. `context`, `project create`, or `code submit` succeeding does not prove that the browser pane has a MagicPath login session.

If opening `/files/<projectId>` in the embedded browser redirects to the MagicPath home or sign-in flow:
...
4. Navigate back to the same project URL after sign-in and continue work on the canvas.

## Project-canvas workflows

### Work in the active project

1. If the user refers to the project they have open, run `npx -y magicpath-ai@experimental context` and read `activeProjects`.
2. If it resolves to one project and it is not already visible in the embedded browser, open that project's `url` there.
3. Keep that project visible while using `context` (for selection), `code start`, and `code submit` for the user's requested work.
...
1. Create the project with `npx -y magicpath-ai@experimental project create --name "<name>"`.
2. Read the returned project `id` and `url`.
3. **Before authoring a design**, in an embedded-browser host open that project `url` in the internal browser.
4. If the URL redirects because the embedded browser is not signed in, complete the browser sign-in flow and reopen the same URL.
5. If no embedded browser is available, run `npx -y magicpath-ai@experimental open <id> --browser` only when user-facing project navigation is needed.
6. Continue the canvas workflow with `code start --project <id> ...`, keeping the project canvas visible for review and selection.
7. After `code submit`, verify progress with the returned `previewImageUrl` and the open project canvas; do not navigate to a single-design preview unless the user expressly asks.

### The user asks for one design

1. If the user asks for a link to an individual design, resolve it with `npx -y magicpath-ai@experimental open <generatedName>` and return its `url`.
...
- Running `context`, `project list`, `component list`, `team list`, `team members`, `search`, `theme list`, or `theme get` only to make an internal decision.
- Using `inspect`, `install`, or other app-integration commands where the requested result is repository code rather than visible canvas work.
- Running automated checks or polling a `code` job before the project contains a viewable result.
...
- **No embedded browser:** Continue with the CLI and use `open <projectId> --browser` for intentional project navigation.

## Boundaries

- Use the main `https://www.magicpath.ai` product surface returned by the CLI.
- Do not replace or reroute `auth login`; authentication owns its browser flow.
...
- Use `context` to connect what the user is looking at in MagicPath back to the agent's next action.
